...
class GoalWidgetProvider : android.appwidget.AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (action) {
            ACTION_COMPLETE_GOAL -> {
                ...
                Log.d(TAG, "Completing goal: $goalId via WorkManager")
                scheduleCompleteGoal(context, goalId)
            }
    ...
    companion object {
        private const val TAG = "GoalWidget"
        const val ACTION_COMPLETE_GOAL = "com.consistencygridwallpaper.ACTION_COMPLETE_GOAL"
        const val ACTION_DATA_CHANGED = "com.consistencygridwallpaper.ACTION_GOAL_WIDGET_DATA_CHANGED"

        fun notifyDataChanged(context: Context) {
            val awm = AppWidgetManager.getInstance(context)
            val ids = awm.getAppWidgetIds(ComponentName(context, GoalWidgetProvider::class.java))
            if (ids.isNotEmpty()) {
        ...
        fun bindWidget(context: Context, appWidgetManager: AppWidgetManager, widgetId: Int) {
            ...
            val templateIntent = Intent(context, GoalWidgetProvider::class.java).apply {
        ...
        /** Schedule WorkManager job to complete a goal — works even if app is killed */
        fun scheduleCompleteGoal(context: Context, goalId: String) {
            try {
                val inputData = Data.Builder()
                    .putString(GoalWidgetActionWorker.KEY_GOAL_ID, goalId)
                    .build()
                val work = OneTimeWorkRequestBuilder<GoalWidgetActionWorker>()
                ...
                Log.d(TAG, "✅ WorkManager job scheduled for goal: $goalId")
            } catch (e: Exception) {
